...
class Grid {
	...
	public:
        ...
        int viable_from(int px, int py, int x, int y, int value, std::set<std::pair<int, int>>& nines) {
            // std::cout << "looking at x=" << x << " y=" << y << " value=" << value << std::endl;
            if (value == 9) {
            ...
            for (auto [dx, dy] : deltas) {
                ...
                if ((*this)[x+dx][y+dy] - '0' == value + 1) {
                    total += viable_from(x, y, x+dx, y+dy, value+1, nines);
                }
            }
            return total;
        }
